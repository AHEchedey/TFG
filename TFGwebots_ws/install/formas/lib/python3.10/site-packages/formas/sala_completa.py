...
class RectangleDrawer(Node):
    def __init__(self):
        ...
        # Lados del rectángulo (en metros)
        ...
        # Ángulo de giro (90 grados en radianes)
    ...
    def curve_right(self, angle: float):
        twist = Twist()
        twist.linear.x = self.linear_speed       # avanzar
        twist.angular.z = -self.angular_speed    # curva a la derecha

        duration = angle / abs(self.angular_speed)
        end_time = self.get_clock().now().nanoseconds / 1e9 + duration
    ...
    def draw_rectangle(self):
        for _ in range(3):  # repetir 3 vueltas
            for distance in self.side_lengths:
                self.move_straight(distance)
                time.sleep(0.5)
                self.curve_right(self.turn_angle)
                time.sleep(0.5)

def main(args=None):
    rclpy.init(args=args)
    node = RectangleDrawer()
    time.sleep(2)  # esperar conexión con cmd_vel
    node.draw_rectangle()
    node.destroy_node()
    rclpy.shutdown()
